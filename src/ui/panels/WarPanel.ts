...
export class WarPanel extends BasePanel {
  ...
  protected renderBody(): string {
    ...
    const goal = `<div>${esc(w.goal.description)}</div>${w.goal.provinces.length ? `<div class="rows" style="margin-top:4px">${w.goal.provinces.slice(0, 10).map((p) => {
      const ctrl = sim.state.provinces[p].controller;
      const held = w.attackers.includes(ctrl);
      return `<div class="row"><span class="grow">${pLink(sim, p)}</span><span class="chip ${held ? 'green' : ''}">${held ? 'ocupado pelos atacantes' : esc(sim.country(ctrl).name)}</span></div>`;
    }).join('')}</div>` : ''}`;
    ...
    const actions = w.active
      ? sec('Intervenção', 'gear', `<div class="action"><button class="px-btn small" data-action="white">${icon('dove', 14)} Paz branca</button><button class="px-btn small" data-action="attackers">Vitória dos atacantes</button><button class="px-btn small" data-action="defenders">Vitória dos defensores</button></div>
      ...
      : '';
    return (
      sec('Placar de guerra', 'scales', marker) +
